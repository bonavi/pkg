...
func CreateNewPassword(password, generalSalt, userSalt []byte) ([]byte, error) {
	...
	if len(password) > maxPasswordLength {
		...
		return nil, errors.Default.New(fmt.Sprintf("Длина пароля не должна превышать %v символов", availableLength))
	}
...
func CompareHashAndPassword(hash, password, userSalt, generalSalt []byte) error {
	...
	if len(password) > maxPasswordLength {
		availableLength := len(password) - len(generalSalt) - saltSize
		return errors.Default.New(fmt.Sprintf("Длина пароля не должна превышать %v символов", availableLength))
	}

	if err := bcrypt.CompareHashAndPassword(hash, SaltPassword(password, userSalt, generalSalt)); err != nil {
		return errors.Default.Wrap(err).WithCustomHumanText("Неверно введен логин или пароль")
	}
	return nil
}
